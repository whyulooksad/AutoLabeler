...
from backend.tracker.model.network import XMem
from backend.tracker.inference.inference_core import InferenceCore
from backend.tracker.util.mask_mapper import MaskMapper
from torchvision import transforms
from backend.tracker.util.range_transform import im_normalization

from backend.tools.painter import mask_painter
from backend.tools.base_segmenter import BaseSegmenter
...
class BaseTracker:
    def __init__(self, xmem_checkpoint, device, sam_model=None, model_type=None) -> None:
        ...
        self.size = int(os.environ["AUTOLABELER_XMEM_SIZE"]) if os.environ.get("AUTOLABELER_XMEM_SIZE") else 0
    ...
    def track(self, frame, first_frame_annotation=None):
        ...
        track_frame, scale = self._resize_frame_for_tracking(frame)
        if first_frame_annotation is not None and scale != 1.0:
            target_w, target_h = track_frame.shape[1], track_frame.shape[0]
            first_frame_annotation = cv2.resize(
                first_frame_annotation.astype(np.uint8),
                (target_w, target_h),
        ...
        if first_frame_annotation is not None:   # first frame mask
            ...
            labels = None
        # prepare inputs
        frame_tensor = self.im_transform(track_frame).to(self.device)
        # track one frame
    ...
    def set_tracking_size(self, size):
        self.size = int(size or 0)

    def _resize_frame_for_tracking(self, frame):
        h, w = frame.shape[:2]
        min_hw = min(h, w)
        if self.size <= 0 or min_hw <= self.size:
            return frame, 1.0

        scale = self.size / min_hw
        target_w = max(16, int(round(w * scale / 16)) * 16)
        target_h = max(16, int(round(h * scale / 16)) * 16)
    ...
    def clear_memory(self):
        ...
        if torch.cuda.is_available():
            try:
                ...
                logger.warning("Failed to clear CUDA cache after tracker cleanup: %s", exc)
