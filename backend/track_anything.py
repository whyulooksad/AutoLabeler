...
from backend.tools.interact_tools import SamControler
from backend.tracker.base_tracker import BaseTracker
from backend.inpainter.base_inpainter import BaseInpainter
import numpy as np
import argparse
...
class TrackingAnything():
    def __init__(self, sam_checkpoint, xmem_checkpoint, e2fgvi_checkpoint, args):
        ...
        # 暂时禁用inpainter以避免mmcv编译问题
        if self.e2fgvi_checkpoint is not None:
            try:
                self.baseinpainter = BaseInpainter(self.e2fgvi_checkpoint, args.device)
            except Exception as e:
                print(f"Warning: Failed to initialize inpainter: {e}")
                self.baseinpainter = None
        else:
            print("Inpainter disabled - e2fgvi_checkpoint is None")
            self.baseinpainter = None 
